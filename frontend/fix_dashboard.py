...
function Dashboard() {
  ...
  const pageStart = (currentPage - 1) * JOBS_PER_PAGE + 1;
  const pageEnd = Math.min(currentPage * JOBS_PER_PAGE, totalJobs);
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-5 gap-5">
            ...
            <StatsCard title="Saved Jobs" value="0" change="Total saved jobs" icon={<FiBookmark />} />
          </div>
              ...
              <div className="bg-[#0B1228] border border-slate-800 rounded-2xl min-h-[650px] p-6">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-slate-400 mt-1">Ranked opportunities based on your profile</p>
                  </div>
                ...
                {error && (
                  <div className="flex items-center justify-center h-64 text-red-400">Failed to load jobs: {error}</div>
                )}
                      ...
                      <table className="w-full">
                        ...
                        <tbody>
                          {paginatedJobs.map((job, index) => {
                            let keywords = [];
                            try {
                              keywords = typeof job.matched_keywords === "string"
                                ...
                                <td>
                                  <div className="flex flex-wrap gap-2">
                                    {keywords.slice(0, 4).map((kw, i) => (
                                      <span key={i} className="px-3 py-1 rounded-full bg-green-900/30 text-green-400 text-xs">{kw}</span>
                                    ))}
                                    {keywords.length > 4 && (
                                      <span className="px-3 py-1 rounded-full bg-slate-700 text-slate-400 text-xs">+{keywords.length - 4} more</span>
                                    )}
                                  </div>
                                </td>
...
path = r"C:\Users\rayan\Desktop\Resumatch\frontend\src\pages\dashboard.jsx"
with open(path, "w", encoding="utf-8") as f:
    f.write(content)
print("dashboard.jsx written successfully.")